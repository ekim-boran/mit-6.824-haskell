...
crasher xs = forever $ do
  ...
  let nodeIds = fmap (\(_, _, _, te) -> fst te) srvs
  randomRIO (0, 5000000) >>= threadDelay
  traverse_ (runTestServer SS.stop) srvs
  liftIO $ print $ "stop servers: " ++ show nodeIds
  randomRIO (0, 500000) >>= threadDelay
  traverse_ (runTestServer SS.start) srvs
...
partitioner network xs = forever $ do
  ...
  ps <- traverse randomPartitions (fmap (\(_, _, _, te) -> fst te) <$> groupOn (\(a, _, _, _) -> gid a) xs)
...
checkStateSize size xs = void $ do
  threadDelay 500000
  s <- maximum <$> traverse (\(_, _, _, te) -> return 0) xs
